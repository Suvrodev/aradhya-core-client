...
const MyProfile = () => {
  ...
  const { data, isLoading } = useGetSpecificStudentQuery(user?.email);
  const loggedStudent: TStudent = data?.data;
  console.log("Student Profile: ", loggedStudent);

  const [profileImage, setProfileImage] = useState(loggedStudent?.image);
  const [isEditing, setIsEditing] = useState(false);
  const toggleEdit = () => setIsEditing(!isEditing);

  const handleAutoUploadImage = async (
    ...
    try {
      ...
      if (uploadedImageUrl) {
        toast.success("Image Uploaded", { id: sonarId });
        const updateData = { image: uploadedImageUrl };

        toast.loading("Updating Profile", { id: sonarId });
        const res = await updateUser({
          email: loggedStudent?.email,
          updateData,
        }).unwrap();
        ...
        if (res?.success) {
          toast.success("Profile Updated", { id: sonarId });
        }
      }
    } catch (error) {
          ...
          <div className="flex flex-col items-center mb-6 relative ">
            <label htmlFor="file-upload" className="cursor-pointer">
              <img
                src={profileImage}
                alt="Profile"
                className="w-24 h-24 rounded-full mb-2 border-2 border-[#00C8FF] hover:opacity-80 transition"
              />
              <Camera className="absolute top-0 right-[60px] md:right-[240px] text-[#00C8FF] bg-gray-800 p-2 rounded-full hover:bg-gray-700 transition size-10" />
            </label>
            <input
              id="file-upload"
              type="file"
              accept="image/*"
              className="hidden"
              onChange={handleAutoUploadImage}
            />
          </div>
